...
llm/client.py's module docstring for why this provider exists at all (tests
only, never the real pipeline).
...
`pytest` into a 10-minute one. Opt in explicitly:
    ...
    RUN_LOCAL_LLM_TESTS=1 poetry run pytest tests/llm/local/
