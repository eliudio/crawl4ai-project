...
from services import feed_importers, parkrun_import
...
def test_fetch_rows_returns_none_when_robots_disallows(monkeypatch, capsys):
    monkeypatch.setattr(parkrun_import.robots, "is_allowed", lambda url, registrator="bot": False)
    monkeypatch.setattr(
...
def _stub_get_or_create_organiser(monkeypatch):
    ...
    organiser.id = 7
    monkeypatch.setattr(feed_importers, "get_or_create_organiser", lambda session, **kw: organiser)
    return organiser
...
def test_run_import_registers_every_event_as_bot(monkeypatch, _stub_get_or_create_organiser):
    ...
    monkeypatch.setattr(parkrun_import.event_crawler, "register_event_from_fields", fake_register)
...
def test_run_import_returns_unusable_summary_when_feed_is_none(monkeypatch, _stub_get_or_create_organiser):
    ...
    monkeypatch.setattr(
        parkrun_import.event_crawler, "register_event_from_fields",
