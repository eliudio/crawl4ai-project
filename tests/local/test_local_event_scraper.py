...
from services.common.models import Event, Organiser
from services.local import local_event_scraper
...
def _stub_new_urls(monkeypatch, urls_by_organiser: dict[int, list[str]]):
    monkeypatch.setattr(
        local_event_scraper, "crawl_listing",
...
def _stub_crawl_event(monkeypatch):
    ...
    monkeypatch.setattr(local_event_scraper, "crawl_event", fake_crawl_event)
    return calls
...
def _stub_crawl_listing_capture(monkeypatch):
    ...
    monkeypatch.setattr(local_event_scraper, "crawl_listing", fake_crawl_listing)
    return captured
