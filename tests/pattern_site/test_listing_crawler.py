...
No real network/LLM calls: firecrawl_client.scrape and the two llm probes are
monkeypatched with canned responses, so these run instantly and for free, and
only exercise listing_crawler's own routing/looping logic. Patched on
`listing_crawler` itself, not `services.llm`/`services.scraping` - listing_crawler
imports each of these as a bare name (`from services.llm import
analyze_listing_page, ...`), so that's the reference it actually calls.
"""
...
from services.pattern_site import listing_crawler
from services.pattern_site.listing_crawler import (
    _APPEND_TEXT_KEYWORDS,
    _find_click_selector,
    _resolve_click_selector,
)
from services.scraping.backends import firecrawl_client
...
def _patch_analyze(monkeypatch):
    monkeypatch.setattr(listing_crawler, "analyze_listing_page", _fake_analyze_listing_page)
...
def test_load_more_site_grows_until_exhausted(monkeypatch, page_url, homepage_url, pages, expected_events):
    ...
    monkeypatch.setattr(
        listing_crawler, "detect_load_more",
...
def test_discover_listing_urls_respects_robots_disallow(monkeypatch, capsys):
    ...
    monkeypatch.setattr(listing_crawler, "is_allowed", lambda url, registrator="bot": False)
    monkeypatch.setattr(
        listing_crawler, "scrape",
...
def test_discover_listing_urls_scrapes_when_allowed(monkeypatch):
    ...
    monkeypatch.setattr(listing_crawler, "is_allowed", lambda url, registrator="bot": True)
    monkeypatch.setattr(
        listing_crawler, "scrape",
    ...
    monkeypatch.setattr(
        listing_crawler, "discover_listing_urls",
...
def test_crawl_listing_dispatches_to_the_registered_handler(monkeypatch):
    ...
    monkeypatch.setattr(listing_crawler, "get_handler", lambda name: fake_handler if name == "custom_handler" else None)
...
def test_crawl_listing_falls_back_to_default_for_unknown_handler_name(monkeypatch, capsys):
    ...
    monkeypatch.setattr(listing_crawler, "get_handler", fake_get_handler)
