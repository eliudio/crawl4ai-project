...
from pattern_site's "Organiser homepage -> listing page -> event page" pipeline (see
README.md's "Feed import pipeline" section). A feed importer owns a whole external
source end to end within one call: fetching it, resolving (or creating) whichever
Organiser row(s) its events belong to, and writing Event rows directly - there's no
separate "discover URLs" vs. "crawl each one" split the other pipeline has, because
there's no per-page scrape at all for a source like this.

Triggered by server/main.py's own /tasks/feed-import Pub/Sub push handler, naming
which importer to run and (optionally) importer-specific params - see
common/pubsub_client.publish_feed_import, and local/local_feed_importer.py for
running one locally without Pub/Sub. Each registered importer is independent and safe
to run concurrently with any other (separate Pub/Sub messages, the same
fan-out-friendly shape the other pipeline's own topics already have).
"""
...
__all__ = ["FeedImporter", "register_importer", "get_importer", "get_or_create_organiser"]
...
def get_or_create_organiser(session: Session, *, name: str, homepage_url: str, discovered_via: str) -> Organiser:
    ...
    Looked up by name, not homepage_url like admin/seed_organisers.py's own CSV-seeding
    ...
    crawl pipeline" contract server/main.py/local/local_event_scraper.py already
    enforce for aggregator/platform rows (see common/models's own SourceType
    docstring: "only rows with source_type == organiser are ever fed into event
    crawling"). That's what keeps this row from ever being picked up by
    crawl_listing() again if a stray listing-crawl message or a bulk re-crawl ever
    names its id - no bespoke check needed anywhere else, this reuses the one that
    already exists.
    """
