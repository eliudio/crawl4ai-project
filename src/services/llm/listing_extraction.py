"""
LLM-backed analysis of an organiser's listing page(s) - finding where listings
live at all, detecting a "load more"/pagination affordance, confirming which
links lead to individual events, and (for sitemaps) picking the one
sub-sitemap that actually lists events. See client.py for the provider
plumbing all of these go through.
"""
...
__all__ = ["discover_listing_urls", "detect_load_more", "select_events_sitemap", "analyze_listing_page"]
...
def detect_load_more(listing_url: str, html: str) -> dict[str, Any]:
    ...
    pattern_site/listing_crawler.py, not variants of one mechanism: 'append'
    (Load More/infinite scroll) grows the same page and is safe to just keep
...
def select_events_sitemap(sitemap_urls: list[str]) -> str | None:
    """
    Used by scraping/sitemap_crawler.py when a robots.txt-advertised sitemap
    turns out to be a <sitemapindex> (a list of OTHER sitemaps - e.g. one for
