...
- crawl_listing() dispatches to whichever handler Organiser.handler names (see
  ...
  admin/discover_sitemaps.py, passed in via handler_params["sitemap_url"]) over
  everything else below, when one is known: scraping/sitemap_crawler.py reads it
  directly - a static XML file, no browser/clicking/per-page LLM confirmation
  needed - and _crawl_from_sitemap only falls through to the mechanisms below
  when no sitemap is known, or the known one couldn't be resolved into anything.
- Failing that, links are first narrowed down heuristically by domain + junk
  ...
  actual event detail pages by the LLM (llm.analyze_listing_page) -
...
- The rest of a listing's events are reached one of three genuinely
  different ways, detected via llm.detect_load_more and handled by
...
from services.common.models import CrawlRun, CrawlRunType, CrawlStatus, Event, Organiser
from services.llm import analyze_listing_page, detect_load_more, discover_listing_urls
from services.scraping import filter_candidate_links, get_event_urls, is_allowed, scrape

from .discovery_handlers import get_handler, register_handler

__all__ = ["crawl_listing"]
...
def _crawl_paginated_same_url(
    ...
    analysis = analyze_listing_page(page_url, first_markdown, candidates)
    ...
    for page_num in range(2, _MAX_LISTING_PAGES + 1):
        ...
        try:
            page_markdown, page_links, _html, _final_url = scrape(
        ...
        page_analysis = analyze_listing_page(page_url, page_markdown, page_candidates)
...
def _discover_listing_urls(session: Session, organiser: Organiser) -> list[str]:
    ...
    """
    if not is_allowed(organiser.homepage_url, registrator=organiser.registrator):
        print(f"ROBOTS-SKIP: {organiser.homepage_url} (listing discovery)")
        return []

    markdown, links, _html, _url = scrape(organiser.homepage_url, want_links=True)
    candidates = filter_candidate_links(links, organiser.homepage_url)
    discovered = discover_listing_urls(organiser.homepage_url, markdown, candidates)
...
def _analyze_page(page_url: str, homepage_url: str) -> tuple[list[str], list[str], str | None]:
    ...
    llm.detect_load_more classifies which of case 2 or case 3 (see
    ...
    affordance is gone, then run llm.analyze_listing_page exactly
    ...
    """
    try:
        markdown, links, html, _url = scrape(page_url, want_links=True, want_html=True)
        probe = detect_load_more(page_url, html)
        ...
        while interaction_type == "append" and round_num < _LOAD_MORE_MAX_ROUNDS:
            ...
            try:
                round_markdown, round_links, round_html, _url = scrape(
        ...
        analysis = analyze_listing_page(page_url, markdown, candidates)
...
def _crawl_one_listing_url(
    ...
    for _ in range(_MAX_LISTING_PAGES):
        ...
        if not is_allowed(page_url, registrator=organiser.registrator):
...
def _filter_new_urls(session: Session, event_urls: list[str], force: bool) -> list[str]:
    ...
    matching local/local_event_scraper.py's --force-refresh contract exactly."""
    if force:
        return event_urls
...
def _crawl_from_sitemap(session: Session, organiser: Organiser, sitemap_url: str, force: bool = False) -> list[str] | None:
    ...
    (see scraping/sitemap_crawler.py) - no browser, no clicking through load-more/pagination, no
    ...
    admin/discover_sitemaps.py has found a Sitemap: entry in this organiser's robots.txt.
    ...
    """
    event_urls = get_event_urls(sitemap_url, organiser.homepage_url, registrator=organiser.registrator)
...
# separate pipeline (see feeds/feed_importers.py/feeds/parkrun_import.py) that never touches this
...
# feeds.feed_importers.get_or_create_organiser's own source_type=PLATFORM, which
# server/main.py/local/local_event_scraper.py both already exclude from this pipeline), it falls
...
def crawl_listing(
    ...
    separate feeds/feed_importers.py pipeline instead, never through this function at all.
    ...
    local/local_event_scraper.py's --force-refresh): the caller is expected to re-crawl each
    ...
    does that; local/local_event_scraper.py's own --mode sanity-check/dry-run gets the same
    ...
    """
    handler = get_handler(organiser.handler)
    if handler is None:
        print(
