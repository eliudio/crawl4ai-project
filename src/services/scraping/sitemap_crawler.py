...
admin/discover_sitemaps.py) as the preferred, direct source of its event
detail page URLs, instead of opening the listing page and clicking through
load-more/pagination (pattern_site/listing_crawler.py's mechanism, still used
as the fallback when no sitemap is known).
...
- A "sitemap index" (<sitemapindex>): a list of OTHER sitemaps (e.g. one
  ...
  (llm.select_events_sitemap) picks which one sounds like it lists
  individual events, judged only from each sub-sitemap's own URL, then that
  one is fetched and used as the url-sitemap above.
"""
...
from services.llm import select_events_sitemap
...
def get_event_urls(sitemap_url: str, homepage_url: str, registrator: str = "bot") -> list[str] | None:
    ...
    if tag == "sitemapindex":
        ...
        events_sitemap = select_events_sitemap(sub_sitemaps)
