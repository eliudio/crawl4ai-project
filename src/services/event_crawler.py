...
def _apply_fields(session: Session, event: Event, fields: dict, registrator: str) -> None:
    """
    Writes an extract_event_fields()-shaped dict onto `event` (and its distances/
    occurrences) - shared by crawl_event's own real LLM-extraction path and
    register_event_from_fields's direct-from-source-data path below, so both stay in
    ...
    - summary_alt/summary_short: if the fields dict already supplies both, used as-is,
      skipping the rewrite_summary LLM call entirely (llm_extractor.extract_event_fields's
      own return value never includes these two keys at all, so the real
      extraction path is unaffected - "not in fields" there, not "falsy").
    - latitude/longitude: if the fields dict already supplies both (non-null), used
      directly, skipping the geocoding_client call entirely - same "key genuinely
      absent for the real extraction path" reasoning.
    """
...
def crawl_event(
    ...
    try:
        ...
        _apply_fields(session, event, fields, registrator)
