...
registration.py calls get_or_create_race_type() once per EventDistance while
...
__all__ = ["get_or_create_race_type"]

_SLUG_RE = re.compile(r"[^a-z0-9]+")

# Deterministic safety net on top of llm/event_extraction's distance_category prompt
# rules -
...
# fold onto the one canonical spelling from llm/event_extraction's named-distance list.
_CATEGORY_ALIASES = {
...
def _coerce_sport(sport_text: str | None) -> Sport:
    """Event.sport is free text straight from the LLM (see llm/event_extraction.py) - map it
    onto RaceType's closed vocabulary, falling back to OTHER rather than failing outright on
    a wording mismatch (e.g. "athletics")."""
    if not sport_text:
        return Sport.OTHER
    try:
