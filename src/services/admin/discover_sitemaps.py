...
pipeline can read that sitemap directly (see scraping/sitemap_crawler.py)
instead of opening the listing page and clicking through load-more/pagination.
...
def discover_sitemaps(csv_path: Path = SEED_CSV) -> None:
    ...
    for row in rows:
        ...
        for attempt in range(1, _RETRIES + 1):
            try:
                # Same identity scraping.robots evaluates this domain's rules
                # against (see scraping/robots.py's own robots.txt fetch) - consistent
