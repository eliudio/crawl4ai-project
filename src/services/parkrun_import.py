...
listing_crawler.py/discovery_handlers.py entirely rather than being another handler
there - it used to be, see git history for the "parkrun" handler this supersedes).
...
from services import event_crawler, feed_importers, robots
...
def build_event_fields(row: dict, today: date | None = None) -> dict | None:
    ...
    # llm_extractor.py's own distance_category docs); 2k takes the general "{n}_k" form.
...
def fetch_rows(registrator: str = "bot") -> list[dict] | None:
    ...
    - same "couldn't resolve this way" contract sitemap_crawler.get_event_urls()/the
    old parkrun_feed.get_event_urls() already use, so callers can tell that apart from
    "read it fine, zero rows" (an empty list).
    """
    if not robots.is_allowed(TSV_URL, registrator=registrator):
...
def get_events(
    ...
    from-source-data shape event_crawler.register_event_from_fields expects. A row
...
def run_import(session: Session, params: dict) -> dict:
    ...
    for event_url, fields in events:
        event_crawler.register_event_from_fields(session, organiser.id, event_url, fields, "bot")

    return {"status": "ok", "registered": len(events), "organiser_id": organiser.id}


feed_importers.register_importer("parkrun", run_import)
