...
def retrieve_md(url: str) -> str:
    """
    Scrape a RunThrough event page and return clean markdown of the main content.
    Handles both old dict-style and new Document-style responses.

    Example: 'https://www.runthrough.co.uk/event/run-dorney-lake-half-marathon-10k-5k-december-2026'
    """
    ...
    try:
        result = app.scrape(
            ...
            exclude_tags=[
                ...
                ".event-map-wrapper",  # remove if you want the map
            ],
            wait_for=3500,  # ms
        )
...
def store_details(page: int, url:str)-> None:
    if not event_with_url_exists(url):
        ...
        print(f"runthrough.co.uk{datetime.now():%H:%M:%S} - skipping: {url} already exists")
