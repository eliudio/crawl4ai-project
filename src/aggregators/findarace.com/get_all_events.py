...
def get_listing_events(listing_url: str) -> list:
    ...
    try:
        ...
        print(f"{datetime.now():%H:%M:%S} - Failed to fetch {listing_url}: {e}")
        return []
...
def process_page(page: int, base_url: str) -> bool:
    if page == 1:
        url = base_url
    else:
        url = f"{base_url}/p{page}"

    print(f"{datetime.now():%H:%M:%S} - Processing page {page}: {url}")

    detail_urls = get_listing_events(url)

    if not detail_urls:
        print(f"{datetime.now():%H:%M:%S} - No events found on page {page} – stopping")
        return False

    print(f"{datetime.now():%H:%M:%S} - Found {len(detail_urls)} potential event URLs on page {page}")

    for detail_url in detail_urls:
        print(f"{datetime.now():%H:%M:%S} - Verifying: {detail_url}")
        store_details(page, detail_url)
